...
class LocalRuntimeConfigTests(TestCase):
    ...
    def test_ci_runs_typecheck_lint_and_mocked_aws_database_checks(self) -> None:
        ...
        self.assertIn("docker compose -f docker-compose.yml up -d postgres", workflow)
        self.assertIn("docker compose -f docker-compose.yml exec -T postgres pg_isready", workflow)
        self.assertIn("run: make typecheck", workflow)
        self.assertIn("run: make lint", workflow)
        self.assertIn("run: make migrate", workflow)
        self.assertIn("run: make seed", workflow)
        self.assertIn("run: make test-local-db", workflow)
        self.assertIn("MIGRATION_DATABASE_URL:", workflow)
        self.assertNotIn("run: make bootstrap-live-db", workflow)
        self.assertNotIn("run: make test-live-db", workflow)
        self.assertNotIn("POSTGRES_CONTAINER_ID", workflow)
    ...
    def test_makefile_uses_sql_mounted_bootstrap_and_role_split(self) -> None:
        makefile = (ROOT / "Makefile").read_text()
        compose = (ROOT / "docker-compose.yml").read_text()
        bootstrap_sql = (
            ROOT / "infra" / "local" / "postgres" / "init" / "001-gridlens-bootstrap.sql"
        ).read_text()
        ...
        self.assertIn("CREATE ROLE gridlens_app", bootstrap_sql)
        self.assertIn("CREATE EXTENSION IF NOT EXISTS vector", bootstrap_sql)
        self.assertIn("CREATE SCHEMA IF NOT EXISTS app AUTHORIZATION gridlens_migrator", bootstrap_sql)
        self.assertIn("ALTER DEFAULT PRIVILEGES FOR ROLE gridlens_migrator", bootstrap_sql)
