...
JWT validation identifies the external subject only. Cognito does not provide
GridLens authorization roles. Product authorization is resolved from GridLens
database rows: `app_users.external_auth_provider = "cognito"` plus
`app_users.external_subject = <cognito sub>`, active
`platform_role_assignments` rows for platform permissions, and the path tenant's
`tenant_memberships` row for tenant permissions.
